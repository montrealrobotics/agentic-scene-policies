...
def depth_to_point_map(
    depth: np.ndarray, intrinsics: np.ndarray, camera_pose: np.ndarray
):
    H, W = depth.shape
    fx, fy, cx, cy = (
        intrinsics[0, 0],
...
def rgbd_to_pcd(
    rgb: np.ndarray, depth: np.ndarray, intrinsics: np.ndarray, camera_pose: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    points = depth_to_point_map(depth, intrinsics, camera_pose)
    rgb = rgb.reshape((-1, 3))
