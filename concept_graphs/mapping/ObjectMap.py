...
class ObjectMap:
    ...
    def union(self, other: "ObjectMap") -> None:
        # Add other objects without merging
        for obj in other:
            self.append(obj)
        self.collate()
    ...
    def filter_points_by_z(self, min_z: float = 0.0, max_z: float = 1.5):
        new_objects = {}
        for k, obj in self.objects.items():
            points = np.asarray(obj.pcd.points)
            mask = (points[:, 2] >= min_z) & (points[:, 2] <= max_z)
            if np.any(mask):
                obj.pcd.points = o3d.utility.Vector3dVector(points[mask])
                obj.pcd.colors = o3d.utility.Vector3dVector(
                    np.asarray(obj.pcd.colors)[mask]
                )
                new_objects[k] = obj
        self.objects = new_objects
        self.collate()

    def filter_large_objects(self, max_size: float = 0.5):
        new_objects = {}
        for k, obj in self.objects.items():
    ...
    def keep_only_object_at_index(self, i: int):
        """Retain only the single object at index i in the ObjectMap and discard the rest."""
        if i < 0 or i >= len(self.key_map):
            raise IndexError(
                f"Index {i} out of range for ObjectMap of length {len(self.key_map)}."
            )

        object_key = self.key_map[i]
        self.objects = {object_key: self.objects[object_key]}
        self.key_map = [object_key]
        self.collate()

    def keep_only_object_at_indices(self, indices: List[int]):
        """Retain only the objects at the indices in the ObjectMap and discard the rest."""

        if not all(0 <= i < len(self.key_map) for i in indices):
            raise IndexError(
                f"Indices {indices} out of range for ObjectMap of length {len(self.key_map)}."
            )
        if len(indices) == 0:
            raise ValueError("No indices provided to keep_only_object_at_indices.")

        self.objects = {self.key_map[i]: self.objects[self.key_map[i]] for i in indices}
        self.key_map = [self.key_map[i] for i in indices]
        self.collate()

    def subset(self, indices: List[int]) -> "ObjectMap":
        """Return a new ObjectMap containing only the objects at the specified indices."""
        if not all(0 <= i < len(self.key_map) for i in indices):
            raise IndexError(
                f"Indices {indices} out of range for ObjectMap of length {len(self.key_map)}."
            )
        if len(indices) == 0:
            raise ValueError("No indices provided to subset.")

        new_map = ObjectMap(
            similarity=copy.deepcopy(self.similarity),
            object_factory=self.object_factory,
            n_min_segments=self.n_min_segments,
            min_points_pcd=self.min_points_pcd,
            grace_min_segments=self.grace_min_segments,
            filter_min_every=self.filter_min_every,
            self_merge_every=self.self_merge_every,
            denoise_every=self.denoise_every,
            downsample_every=self.downsample_every,
            device=self.device,
        )

        for i in indices:
            new_map.append(copy.deepcopy(self[i]))
    ...
    def export(self, path: str) -> None:
        ...
        for obj_id, obj in enumerate(self):
            n_points_object = len(obj.pcd.points)
            pcd_merged += obj.pcd

            # ---- shift local -> global for each instance ----
            # ---- and strip big / non‑serialisable fields ----
            export_instances = {}
            for inst_id, inst in obj.affordance_instances.items():
                if inst.mask_3d_indices_local is not None:
                    inst.mask_3d_indices_global = (
                        inst.mask_3d_indices_local + point_counter
                    ).astype(int)

                export_instances[int(inst_id)] = dict(
                    tag=inst.tag,
                    type=inst.type,
                    score=inst.score,
                    interaction_points=[list(pt) for pt in inst.interaction_points],
                    mask_3d_indices_global=(
                        inst.mask_3d_indices_global.tolist()
                        if inst.mask_3d_indices_global is not None
                        else []
                    ),
                    desc_id=inst.desc_id,
                )
            ...
            obj_ann = dict(
                ...
                affordance_instances=export_instances,
            )
    ...
    def query(self, query_ft: torch.Tensor) -> torch.Tensor | None:
        """
        Compare object features with query_ft. Return similarities
        """
        if self.semantic_tensor is None:
            return None

        query_ft = query_ft.to(self.device)
        sim_objects = self.similarity.semantic_similarity(
            query_ft, self.semantic_tensor
        )
